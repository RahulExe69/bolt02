...
const Snake: React.FC = () => {
  ...
  const moveSnake = useCallback(() => {
    ...
    if (foodIndex !== -1) {
      ...
      if (newScore >= WALL_SPAWN_SCORE && newScore % WALL_SPAWN_SCORE === 0) {
  ...
  }, [state.nextDirection, state.foods, state.walls, state.snake]);
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-4xl mx-auto p-4">
      ...
      <div className="mt-8 text-center text-sm text-gray-600 dark:text-gray-400">
        ...
        <p className="mt-2">Watch out for walls that appear after score {WALL_SPAWN_SCORE}!</p>
      </div>
    </div>
  );
};
